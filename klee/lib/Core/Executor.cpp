...
#ifdef SASE
void Executor::updateSemanticTrace(
  ExecutionState& state,
  KInstruction* ki)
{
  auto bb=
  ...
  auto it=
      guidance.bbTagMap.find(bbid);

  if(it!=guidance.bbTagMap.end())
  {
      frame.trace.tags.push_back(
          it->second);

      frame.instructionsSinceLastTag=0;
  }
  else
  {
      frame.instructionsSinceLastTag++;
  }
}
#endif
